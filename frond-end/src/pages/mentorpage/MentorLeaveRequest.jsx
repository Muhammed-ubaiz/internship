...
function MentorLeaveRequest() {
  ...
  const handleStatusChange = async (id, action) => {
    try {
      const res = await api.post(`/mentor/leave-requests/${id}/${action}`);
      const data = res.data;
              ...
              <div className="flex flex-wrap items-center gap-2 px-3 py-2 bg-blue-50 rounded-lg">
                ...
                {leaveTypeFilter !== "all" && (
                  <span className="inline-flex items-center gap-1 px-2 py-1 text-xs bg-blue-100 text-blue-700 rounded-full">
                    ...
                    <button onClick={() => setLeaveTypeFilter("all")} className="ml-1 hover:text-blue-900">
                      <X className="w-3 h-3" />
                    </button>
                  </span>
                )}
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-2xl sm:rounded-3xl shadow-xl border p-8 sm:p-12 text-center">
              ...
              <h3 className="text-lg sm:text-xl font-semibold text-gray-700 mb-2">
                {leaveRequests.length === 0 ? "No Pending Requests" : "No Matching Requests"}
              </h3>
                ...
                {filteredRequests.map((leave) => (
                  <div key={leave._id} className="bg-[#EEF6FB] rounded-xl p-4 transform transition-all duration-300">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <span className={`px-2 py-0.5 text-xs font-medium rounded-full border ${getLeaveTypeColor(leave.leaveType)}`}>
                            {leave.leaveType}
                          </span>
                        </div>
                      </div>
                      <span className="px-2 py-1 text-xs font-semibold rounded-full bg-yellow-50 text-yellow-700 border border-yellow-200">Pending</span>
                    </div>

                    <div className="flex items-center gap-2 text-xs text-gray-500 mb-3">
                      <Mail className="w-3 h-3" />
                      <span className="truncate">{leave.studentEmail}</span>
                    </div>
                    ...
                    <div className="flex justify-end gap-2">
                      <button onClick={() => handleStatusChange(leave._id, "rejected")} className="px-3 py-1.5 text-xs font-medium rounded-lg border border-red-600 text-red-600 hover:bg-red-50 transition-colors flex items-center gap-1">
                        <XCircle className="w-3 h-3" />
                        Reject
                      </button>
                      <button onClick={() => handleStatusChange(leave._id, "approved")} className="px-3 py-1.5 text-xs font-medium rounded-lg bg-[#0a2540] text-white hover:bg-[#0a2540]/90 transition-colors flex items-center gap-1">
                    ...
                    <div key={leave._id} className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
                      ...
                      <div className="p-6 border-b border-gray-100">
                        <div className="flex items-start justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <div className="flex items-center gap-4 mt-1">
                                <span className={`px-3 py-1 text-xs font-medium rounded-full border ${getLeaveTypeColor(leave.leaveType)}`}>
                                  {leave.leaveType}
                                </span>
                          ...
                          <span className="px-3 py-1 text-xs font-semibold rounded-full bg-yellow-50 text-yellow-700 border border-yellow-200">Pending</span>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-6">
                          <div className="space-y-2">
                            ...
                            <p className="font-medium text-gray-900">
                              {new Date(leave.fromDate).toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric", year: "numeric" })}
                            </p>
                          </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <p className="font-medium text-gray-900">
                              {new Date(leave.toDate).toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric", year: "numeric" })}
                            </p>
                          </div>
                        </div>
                      ...
                      <div className="p-6 border-t border-gray-100 bg-gray-50">
                        <div className="flex justify-end gap-3">
                          <button onClick={() => handleStatusChange(leave._id, "rejected")} className="px-5 py-2.5 text-sm font-medium rounded-lg border border-red-600 text-red-600 hover:bg-red-50 transition-colors flex items-center gap-2">
                            <XCircle className="w-4 h-4" />
                            Reject
                          </button>
                          <button onClick={() => handleStatusChange(leave._id, "approved")} className="px-5 py-2.5 text-sm font-medium rounded-lg bg-[#0a2540] text-white hover:bg-[#0a2540]/90 transition-colors flex items-center gap-2">
            ...
            <div className="mt-8 bg-white rounded-xl shadow-sm border p-4">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-sm">
                <div className="text-gray-500">
                  {searchTerm || leaveTypeFilter !== "all" ? (
                    <>
                      Found <span className="font-semibold text-[#0a2540]">{filteredRequests.length}</span> result{filteredRequests.length !== 1 ? "s" : ""}
                      {searchTerm && <> for "<span className="font-semibold">{searchTerm}</span>"</>}
                      {leaveTypeFilter !== "all" && <> with type "<span className="font-semibold">{leaveTypeFilter}</span>"</>}
                    </>
                  ) : (
                    <>
                      Showing all <span className="font-semibold">{filteredRequests.length}</span> pending leave requests
                    </>
                  )}
                </div>
